...
func runBOF(implant ActiveImplant, rpc rpcpb.RevilsRPCClient, bof []byte, args []bofArgs, onFinishCallback func(string, int, int)) (output string, err error) {
	...
	extResp, err := rpc.CallExtension(context.Background(), &revilspb.CallExtensionReq{
		Name:        coffLoaderName,
		ServerStore: false,
		Args:        extArgsBuffer,
...
func registerLoader(implant ActiveImplant, rpc rpcpb.RevilsRPCClient) error {
	...
	resp, err := rpc.RegisterExtension(context.Background(), &revilspb.RegisterExtensionReq{
		Name:    coffLoaderName,
		Data:    loaderData,
		OS:      implant.GetOS(),
		Init:    "",
...
func isLoaderLoaded(implant ActiveImplant, rpc rpcpb.RevilsRPCClient) bool {
	extList, err := rpc.ListExtensions(context.Background(), &revilspb.ListExtensionsReq{
		Request: MakeRequest(implant),
	})
	if err != nil {
		return false
