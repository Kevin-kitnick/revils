...
import (
	...
	"github.com/Kevin-kitnick/revils/client/spin"
...
type RevilsConsoleClient struct {
	App                      *grumble.App
	Rpc                      rpcpb.RevilsRPCClient
...
type BindCmds func(console *RevilsConsoleClient)

// Start - Console entrypoint
func Start(rpc rpcpb.RevilsRPCClient, bindCmds BindCmds, extraCmds BindCmds, isServer bool) error {
	assets.Setup(false, false)
	settings, _ := assets.LoadSettings()
	con := &RevilsConsoleClient{
		App: grumble.New(&grumble.Config{
			Name:                  "Revils",
			Description:           "Revils Client",
...
func (con *RevilsConsoleClient) startEventLoop() {
...
func (con *RevilsConsoleClient) CreateEventListener() (string, <-chan *clientpb.Event) {
...
func (con *RevilsConsoleClient) RemoveEventListener(listenerID string) {
...
func (con *RevilsConsoleClient) triggerEventListeners(event *clientpb.Event) {
...
func (con *RevilsConsoleClient) triggerReactions(event *clientpb.Event) {
...
func (con *RevilsConsoleClient) triggerBeaconTaskCallback(data []byte) {
...
func (con *RevilsConsoleClient) AddBeaconCallback(taskID string, callback BeaconTaskCallback) {
	con.BeaconTaskCallbacksMutex.Lock()
	defer con.BeaconTaskCallbacksMutex.Unlock()
	con.BeaconTaskCallbacks[taskID] = callback
}

func (con *RevilsConsoleClient) GetPrompt() string {
	prompt := Underline + "revils" + Normal
	if con.IsServer {
		prompt = Bold + "[server] " + Normal + Underline + "revils" + Normal
	}
...
func (con *RevilsConsoleClient) PrintLogo() {
	...
	con.Println(Info + "Welcome to the revils shell, please type 'help' for options")
	con.Println()
...
func (con *RevilsConsoleClient) CheckLastUpdate() {
	now := time.Now()
...
func (con *RevilsConsoleClient) GetSession(arg string) *clientpb.Session {
...
func (con *RevilsConsoleClient) GetSessionsByName(name string) []*clientpb.Session {
	sessions, err := con.Rpc.GetSessions(context.Background(), &commonpb.Empty{})
	if err != nil {
		fmt.Printf(Warn+"%s\n", err)
		return nil
...
func (con *RevilsConsoleClient) GetActiveSessionConfig() *clientpb.ImplantConfig {
...
func (con *RevilsConsoleClient) PrintAsyncResponse(resp *commonpb.Response) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func (con *RevilsConsoleClient) Printf(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stdout(), format, args...)
}

func (con *RevilsConsoleClient) Println(args ...interface{}) (n int, err error) {
	return fmt.Fprintln(con.App.Stdout(), args...)
}

func (con *RevilsConsoleClient) PrintInfof(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stdout(), Clearln+Info+format, args...)
}

func (con *RevilsConsoleClient) PrintSuccessf(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stdout(), Clearln+Success+format, args...)
}

func (con *RevilsConsoleClient) PrintWarnf(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stdout(), Clearln+"⚠️  "+Normal+format, args...)
}

func (con *RevilsConsoleClient) PrintErrorf(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stderr(), Clearln+Warn+format, args...)
}

func (con *RevilsConsoleClient) PrintEventInfof(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stdout(), Clearln+Info+format+"\n"+Clearln+"\r\n"+Clearln+"\r", args...)
}

func (con *RevilsConsoleClient) PrintEventErrorf(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stderr(), Clearln+Warn+format+"\n"+Clearln+"\r\n"+Clearln+"\r", args...)
}

func (con *RevilsConsoleClient) PrintEventSuccessf(format string, args ...interface{}) (n int, err error) {
	return fmt.Fprintf(con.App.Stdout(), Clearln+Success+format+"\n"+Clearln+"\r\n"+Clearln+"\r", args...)
}

func (con *RevilsConsoleClient) SpinUntil(message string, ctrl chan bool) {
...
func (con *RevilsConsoleClient) FormatDateDelta(t time.Time, includeDate bool, color bool) string {
