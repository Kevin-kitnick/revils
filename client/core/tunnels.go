...
type tunnels struct {
	...
	mutex       *sync.RWMutex
	streamMutex *sync.Mutex
	stream      rpcpb.RevilsRPC_TunnelDataClient
}

func (t *tunnels) SetStream(stream rpcpb.RevilsRPC_TunnelDataClient) {
	t.streamMutex.Lock()
...
func (t *tunnels) send(tunnelData *revilspb.TunnelData) error {
	t.streamMutex.Lock()
	defer t.streamMutex.Unlock()
...
func (t *tunnels) Start(tunnelID uint64, sessionID string) *TunnelIO {
	...
	go func(tunnel *TunnelIO) {
		...
		for data := range tunnel.Send {
			...
			err := t.send(&revilspb.TunnelData{
				TunnelID:  tunnel.ID,
