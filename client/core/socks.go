...
import (
	...
	"github.com/Kevin-kitnick/revils/util/leaky"
)
...
func (f *socksProxy) Start(tcpProxy *TcpProxy) error {
	...
	for {
		...
		rpcSocks, err := tcpProxy.Rpc.CreateSocks(context.Background(), &revilspb.Socks{
		...
		go connect(connection, proxy, &revilspb.SocksData{
...
func connect(conn net.Conn, stream rpcpb.RevilsRPC_SocksProxyClient, frame *revilspb.SocksData) {
