...
// ChannelProxy binds the Revils Tunnel to a net.Conn object
...
func (p *ChannelProxy) dialImplant(ctx context.Context) (*TunnelIO, error) {
	...
	rpcTunnel, err := p.Rpc.CreateTunnel(ctx, &revilspb.Tunnel{
	...
	portfwdResp, err := p.Rpc.Portfwd(ctx, &revilspb.PortfwdReq{
		Request: &commonpb.Request{
			SessionID: p.Session.ID,
		},
		Host:     p.Host(),
		Port:     p.Port(),
		Protocol: revilspb.PortFwdProtoTCP,
		TunnelID: tunnel.ID,
	})
	if err != nil {
		return nil, err
	...
	if portfwdResp.Response != nil && portfwdResp.Response.Err != "" {
		p.Rpc.CloseTunnel(ctx, &revilspb.Tunnel{
			TunnelID:  tunnel.ID,
			SessionID: p.Session.ID,
		})
