...
func getType(t string) (uint32, error) {
	...
	switch t {
	case "binary":
		res = revilspb.RegistryTypeBinary
	case "dword":
		res = revilspb.RegistryTypeDWORD
	case "qword":
		res = revilspb.RegistryTypeQWORD
	case "string":
		res = revilspb.RegistryTypeString
	default:
...
func RegReadCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
	var (
		finalPath string
		key       string
	)
	...
	regRead, err := con.Rpc.RegistryRead(context.Background(), &revilspb.RegistryReedReq{
		Hive:     hive,
		Path:     finalPath,
		Key:      key,
		Hostname: hostname,
...
func PrintRegRead(regRead *revilspb.RegistryRead, con *console.RevilsConsoleClient) {
