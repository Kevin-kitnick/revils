...
// AliasesLoadCmd - Locally load a alias into the Revils shell.
func AliasesLoadCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
...
// LoadAlias - Load an alias into the Revils shell from a given directory
func LoadAlias(manifestPath string, con *console.RevilsConsoleClient) (*AliasManifest, error) {
	...
	longHelpMsg += "https://github.com/BishopFox/revils/wiki/Aliases-&-Extensions#aliases-command-parsing"
...
func runAliasCommand(ctx *grumble.Context, con *console.RevilsConsoleClient) {
	...
	if aliasManifest.IsAssembly {
		...
		executeAssemblyResp, err := con.Rpc.ExecuteAssembly(context.Background(), &revilspb.ExecuteAssemblyReq{
		...
		msg := fmt.Sprintf("Executing %s %s ...", ctx.Command.Name, extArgs)
		con.SpinUntil(msg, ctrl)
		spawnDllResp, err := con.Rpc.SpawnDll(context.Background(), &revilspb.InvokeSpawnDlllReq{
			Request:     con.ActiveTarget.Request(ctx),
		...
		msg := fmt.Sprintf("Executing %s %s ...", ctx.Command.Name, extArgs)
		con.SpinUntil(msg, ctrl)
		sideloadResp, err := con.Rpc.Sideload(context.Background(), &revilspb.SideloadReq{
			Request:     con.ActiveTarget.Request(ctx),
			Args:        extArgs,
			Data:        binData,
...
func PrintSpawnDLLOutput(cmdName string, spawnDllResp *revilspb.SpawnDll, outFilePath *os.File, con *console.RevilsConsoleClient) {
...
func PrintSideloadOutput(cmdName string, sideloadResp *revilspb.Sideload, outFilePath *os.File, con *console.RevilsConsoleClient) {
...
func PrintAssemblyOutput(cmdName string, execAsmResp *revilspb.ExecuteAssembly, outFilePath *os.File, con *console.RevilsConsoleClient) {
