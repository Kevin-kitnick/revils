...
func ImplantsCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
...
func PrintImplantBuilds(configs map[string]*clientpb.ImplantConfig, filters ImplantBuildFilter, con *console.RevilsConsoleClient) {
	tw := table.NewWriter()
	...
	for revilsName, config := range configs {
...
func ImplantBuildNameCompleter(prefix string, args []string, filters ImplantBuildFilter, con *console.RevilsConsoleClient) []string {
	builds, err := con.Rpc.ImplantBuilds(context.Background(), &commonpb.Empty{})
	if err != nil {
		return []string{}
	}
...
func ImplantBuildByName(name string, con *console.RevilsConsoleClient) *clientpb.ImplantConfig {
	builds, err := con.Rpc.ImplantBuilds(context.Background(), &commonpb.Empty{})
	if err != nil {
		return nil
	}
	for revilsName, build := range builds.Configs {
		if revilsName == name {
			return build
		}
	}
