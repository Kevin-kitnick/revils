...
func PsCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
...
func PrintPS(os string, ps *revilspb.Ps, interactive bool, ctx *grumble.Context, con *console.RevilsConsoleClient) {
...
func findKnownSecurityProducts(ps *revilspb.Ps) []string {
	products := []string{}
	for _, proc := range ps.Processes {
...
func procRow(tw table.Writer, proc *commonpb.Process, cmdLine bool, con *console.RevilsConsoleClient) table.Row {
	session, beacon := con.ActiveTarget.GetInteractive()
...
func GetPIDByName(ctx *grumble.Context, name string, con *console.RevilsConsoleClient) int {
	ps, err := con.Rpc.Ps(context.Background(), &revilspb.PsReq{
		Request: con.ActiveTarget.Request(ctx),
	})
	if err != nil {
