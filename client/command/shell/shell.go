...
func ShellCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
	...
	if con.ActiveTarget.GetSession().OS != linux && con.ActiveTarget.GetSession().OS != darwin {
		noPty = true // Revils's PTYs are only supported on linux/darwin
	}
	runInteractive(ctx, shellPath, noPty, con)
	con.Println("Shell exited")
}

func runInteractive(ctx *grumble.Context, shellPath string, noPty bool, con *console.RevilsConsoleClient) {
	con.Println()
	...
	rpcTunnel, err := con.Rpc.CreateTunnel(ctxTunnel, &revilspb.Tunnel{
	...
	if shell.Response != nil && shell.Response.Err != "" {
		...
		_, err = con.Rpc.CloseTunnel(context.Background(), &revilspb.Tunnel{
			TunnelID:  tunnel.ID,
			SessionID: session.ID,
		})
		if err != nil {
