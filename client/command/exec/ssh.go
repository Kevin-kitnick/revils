...
func SSHCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
	var (
		privKey       []byte
		signeUserCert []byte
		err           error
	...
	sshCmd, err := con.Rpc.RunSSHCommand(context.Background(), &revilspb.SSHCommandReq{
		Username:       username,
		Hostname:       hostname,
		Port:           uint32(port),
		PrivKey:        privKey,
...
func PrintSSHCmd(sshCmd *revilspb.SSHCommand, con *console.RevilsConsoleClient) {
...
func tryCredsFromLoot(con *console.RevilsConsoleClient) (string, string, []byte) {
	var (
		username string
		password string
		privKey  []byte
