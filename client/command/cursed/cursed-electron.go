...
func CursedElectronCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
...
func avadaKedavraElectron(electronExe string, session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) *core.CursedProcess {
	...
	if electronProcess != nil {
		...
		con.PrintWarnf("Revils will need to kill and restart the process in order to perform code injection.\n")
...
func checkElectronPath(electronExe string, session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (bool, error) {
...
func checkElectronProcess(electronExe string, session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (*commonpb.Process, error) {
...
func startCursedElectronProcess(electronExe string, session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (*core.CursedProcess, error) {
	...
	electronExec, err := con.Rpc.Execute(context.Background(), &revilspb.ExecuteReq{
		Request: con.ActiveTarget.Request(ctx),
		Path:    electronExe,
		Args:    args,
		Output:  false,
...
func getElectronProcess(session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (*commonpb.Process, error) {
	ps, err := con.Rpc.Ps(context.Background(), &revilspb.PsReq{
		Request: con.ActiveTarget.Request(ctx),
	})
	if err != nil {
