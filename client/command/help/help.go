...
func HelpCmd(con *console.RevilsConsoleClient) func(a *grumble.App, shell bool) {
	return func(a *grumble.App, shell bool) {
		printHelp(con)
	}
}

func printHelp(con *console.RevilsConsoleClient) {
	...
	for _, c := range con.App.Commands().All() {
		...
		if beacon != nil || session != nil {
			if targetOS != "windows" && key == consts.RevilsWinHelpGroup {
				continue
			}
		} else {
			if key == consts.RevilsHelpGroup || key == consts.RevilsWinHelpGroup || key == consts.AliasHelpGroup || key == consts.ExtensionHelpGroup {
				continue
			}
		}
		if len(key) == 0 {
	...
	con.Println()
	con.Printf("For even more information, please see our wiki: https://github.com/BishopFox/revils/wiki\n")
	con.Println()
}

func headlinePrinter(c *grumble.Config, con *console.RevilsConsoleClient) func(v ...interface{}) (int, error) {
...
func printHeadline(config *grumble.Config, s string, con *console.RevilsConsoleClient) {
