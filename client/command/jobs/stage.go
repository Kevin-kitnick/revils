...
func StageListenerCmd(ctx *grumble.Context, con *console.RevilsConsoleClient) {
	...
	stage2, err := generate.GetRevilsBinary(profile, con)
	if err != nil {
		con.PrintErrorf("%s\n", err)
		return
	}
	...
	case "http":
		...
		stageListener, err := con.Rpc.StartHTTPStagerListener(context.Background(), &clientpb.StagerListenerReq{
			...
			ProfileName: fmt.Sprintf("%s (Revils name: %s)", profileName,
