...
#            Revils using this container configuration! However, if you do want
...
# > User
#
RUN groupadd -g 999 revils && useradd -r -u 999 -g revils revils
RUN mkdir -p /home/revils/ && chown -R revils:revils /home/revils
...
RUN mkdir -p ~/.msf4/ && touch ~/.msf4/initial_setup_complete \
    &&  su -l revils -c 'mkdir -p ~/.msf4/ && touch ~/.msf4/initial_setup_complete'
...
# Go assets
WORKDIR /go/src/github.com/Kevin-kitnick/revils
ADD . /go/src/github.com/Kevin-kitnick/revils/
RUN make clean-all
RUN make \
    && cp -vv revils-server /opt/revils-server \
    && /opt/revils-server unpack --force 

# Run unit tests
RUN /go/src/github.com/Kevin-kitnick/revils/go-tests.sh

# Clean up
RUN make clean \
    && rm -rf /go/src/* \
    && rm -rf /home/revils/.revils

USER revils
WORKDIR /home/revils/
ENTRYPOINT [ "/opt/revils-server" ]
