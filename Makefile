...
VERSION_PKG = github.com/Kevin-kitnick/revils/client/version
CLIENT_ASSETS_PKG = github.com/Kevin-kitnick/revils/client/assets
...
default: clean .downloaded_assets validate-go-version
	$(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),server $(LDFLAGS) -o revils-server$(ARTIFACT_SUFFIX) -buildvcs=false ./server
	$(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),client $(LDFLAGS) -o revils-client$(ARTIFACT_SUFFIX) -buildvcs=false ./client

.PHONY: macos
macos: clean .downloaded_assets validate-go-version
	GOOS=darwin GOARCH=amd64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),server $(LDFLAGS) -o revils-server$(ARTIFACT_SUFFIX) ./server
	GOOS=darwin GOARCH=amd64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),client $(LDFLAGS) -o revils-client$(ARTIFACT_SUFFIX) ./client

.PHONY: macos-arm64
macos-arm64: clean .downloaded_assets validate-go-version
	GOOS=darwin GOARCH=arm64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),server $(LDFLAGS) -o revils-server$(ARTIFACT_SUFFIX) ./server
	GOOS=darwin GOARCH=arm64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),client $(LDFLAGS) -o revils-client$(ARTIFACT_SUFFIX) ./client

.PHONY: linux
linux: clean .downloaded_assets validate-go-version
	GOOS=linux GOARCH=amd64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),server $(LDFLAGS) -o revils-server$(ARTIFACT_SUFFIX) ./server
	GOOS=linux GOARCH=amd64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),client $(LDFLAGS) -o revils-client$(ARTIFACT_SUFFIX) ./client

.PHONY: linux-arm64
linux-arm64: clean .downloaded_assets validate-go-version
	GOOS=linux GOARCH=arm64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),server $(LDFLAGS) -o revils-server$(ARTIFACT_SUFFIX) ./server
	GOOS=linux GOARCH=arm64 $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),client $(LDFLAGS) -o revils-client$(ARTIFACT_SUFFIX) ./client

.PHONY: windows
windows: clean .downloaded_assets validate-go-version
	GOOS=windows $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),server $(LDFLAGS) -o revils-server$(ARTIFACT_SUFFIX).exe ./server
	GOOS=windows $(ENV) $(GO) build -mod=vendor -trimpath $(TAGS),client $(LDFLAGS) -o revils-client$(ARTIFACT_SUFFIX).exe ./client

.PHONY: pb
pb:
	protoc -I protobuf/ protobuf/commonpb/common.proto --go_out=paths=source_relative:protobuf/
	protoc -I protobuf/ protobuf/revilspb/revils.proto --go_out=paths=source_relative:protobuf/
...
.PHONY: debug
debug: clean
	$(ENV) $(GO) build -mod=vendor $(TAGS),server $(LDFLAGS_DEBUG) -o revils-server$(ARTIFACT_SUFFIX) ./server
	$(ENV) $(GO) build -mod=vendor $(TAGS),client $(LDFLAGS_DEBUG) -o revils-client$(ARTIFACT_SUFFIX) ./client
...
.PHONY: clean
clean:
	rm -f revils-client revils-server revils-*.exe
