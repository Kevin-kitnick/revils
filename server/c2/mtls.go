...
func StartMutualTLSListener(bindIface string, port uint16) (net.Listener, error) {
	...
	go acceptRevilsConnections(ln)
	return ln, nil
}

func acceptRevilsConnections(ln net.Listener) {
	for {
		conn, err := ln.Accept()
		if err != nil {
		...
		go handleRevilsConnection(conn)
	}
}

func handleRevilsConnection(conn net.Conn) {
...
func socketWriteEnvelope(connection net.Conn, envelope *revilspb.Envelope) error {
...
func socketReadEnvelope(connection net.Conn) (*revilspb.Envelope, error) {
