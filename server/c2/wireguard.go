...
import (
	...
	serverHandlers "github.com/Kevin-kitnick/revils/server/handlers"
	"github.com/Kevin-kitnick/revils/server/log"
	"github.com/Kevin-kitnick/revils/server/netstack"
...
func StartWGListener(port uint16, netstackPort uint16, keyExchangeListenPort uint16) (net.Listener, *device.Device, *bytes.Buffer, error) {
	...
	if err != nil {
		wgLog.Errorf("Failed to setup up wg revils listener: %v", err)
		return nil, nil, nil, err
	}
	wgLog.Printf("Successfully setup up wg revils listener")
	go acceptWGRevilsConnections(listener)
...
func acceptWGRevilsConnections(ln net.Listener) {
	for {
		conn, err := ln.Accept()
		if err != nil {
			if errType, ok := err.(*net.OpError); ok && errType.Op == "accept" {
		...
		go handleWGRevilsConnection(conn)
	}
}

func handleWGRevilsConnection(conn net.Conn) {
...
func socketWGWriteEnvelope(connection net.Conn, envelope *revilspb.Envelope) error {
...
func socketWGReadEnvelope(connection net.Conn) (*revilspb.Envelope, error) {
