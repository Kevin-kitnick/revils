...
var (
	builderLog = log.NamedLogger("builder", "revils")
)
...
func StartBuilder(externalBuilder *clientpb.Builder, rpc rpcpb.RevilsRPCClient) {
...
func buildEvents(externalBuilder *clientpb.Builder, rpc rpcpb.RevilsRPCClient) (<-chan *clientpb.Event, error) {
...
func handleBuildEvent(externalBuilder *clientpb.Builder, event *clientpb.Event, rpc rpcpb.RevilsRPCClient) {
	...
	if extConfig.Config.TemplateName != "revils" {
	...
	case clientpb.OutputFormat_EXECUTABLE:
		fPath, err = generate.RevilsExecutable(extConfig.Config.Name, extConfig.OTPSecret, extModel, false)
	case clientpb.OutputFormat_SHARED_LIB:
		fPath, err = generate.RevilsSharedLibrary(extConfig.Config.Name, extConfig.OTPSecret, extModel, false)
	case clientpb.OutputFormat_SHELLCODE:
		fPath, err = generate.RevilsShellcode(extConfig.Config.Name, extConfig.OTPSecret, extModel, false)
	default:
	...
	if err != nil {
		builderLog.Errorf("Failed to generate revils: %s", err)
		rpc.BuilderTrigger(context.Background(), &clientpb.Event{
			EventType: consts.ExternalBuildFailedEvent,
			Data:      []byte(fmt.Sprintf("%s:%s", implantConfigID, err.Error())),
		})
	...
	if err != nil {
		builderLog.Errorf("Failed to read generated revils: %s", err)
		rpc.BuilderTrigger(context.Background(), &clientpb.Event{
			EventType: consts.ExternalBuildFailedEvent,
			Data:      []byte(fmt.Sprintf("%s:%s", implantConfigID, err.Error())),
		})
