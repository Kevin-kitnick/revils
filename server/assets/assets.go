...
import (
	...
	ver "github.com/Kevin-kitnick/revils/client/version"
	protobufs "github.com/Kevin-kitnick/revils/protobuf"
...
// GetRootAppDir - Get the Revils app dir, default is: ~/.revils/
func GetRootAppDir() string {
	...
	if len(value) == 0 {
		user, _ := user.Current()
		dir = path.Join(user.HomeDir, ".revils")
	} else {
		dir = value
	}
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		...
		if err != nil {
			setupLog.Fatalf("Cannot write to revils root dir %s", err)
		}
	}
	return dir
}
...
func Setup(force bool, echo bool) {
	...
	if force || localVer == "" || localVer != ver.GitCommit {
		...
		if echo {
			fmt.Printf(`
Revils  Copyright (C) 2022  Bishop Fox
...
func GetGPGPublicKey() (*packet.PublicKey, error) {
	rawPublicKey, err := assetsFs.ReadFile("fs/revils.asc")
	if err != nil {
		return nil, err
	}
...
func SetupGoPath(goPathSrc string) error {
	...
	// Revils PB
	revilspbGoSrc, err := protobufs.FS.ReadFile("revilspb/revils.pb.go")
	if err != nil {
		setupLog.Info("Static asset not found: revils.pb.go")
		return err
	}
	revilspbConstSrc, err := protobufs.FS.ReadFile("revilspb/constants.go")
	if err != nil {
		setupLog.Info("Static asset not found: constants.go")
		return err
	}
	revilspbGoSrc = xorPBRawBytes(revilspbGoSrc)
	revilspbGoSrc = stripRevilspb(revilspbGoSrc)
	revilspbDir := filepath.Join(goPathSrc, "github.com", "Kevin-kitnick", "revils", "protobuf", "revilspb")
	os.MkdirAll(revilspbDir, 0700)
	os.WriteFile(filepath.Join(revilspbDir, "revils.pb.go"), revilspbGoSrc, 0600)
	os.WriteFile(filepath.Join(revilspbDir, "constants.go"), revilspbConstSrc, 0600)
	...
	commonpbDir := filepath.Join(goPathSrc, "github.com", "Kevin-kitnick", "revils", "protobuf", "commonpb")
	...
	dnspbDir := filepath.Join(goPathSrc, "github.com", "Kevin-kitnick", "revils", "protobuf", "dnspb")
	os.MkdirAll(dnspbDir, 0700)
	os.WriteFile(filepath.Join(dnspbDir, "dns.pb.go"), dnspbSrc, 0600)
	return nil
}

func stripRevilspb(src []byte) []byte {
	out := src
...
func xorPBRawBytes(src []byte) []byte {
	var (
		fileAst                   *ast.File
		err                       error
		revilspbVarName           = "file_revilspb_revils_proto_rawDesc"
		revilsPbProtoInitFuncName = "file_revilspb_revils_proto_init"
	...
	ast.Inspect(fileAst, func(n ast.Node) bool {
		...
		case *ast.FuncDecl:
			...
			// that calls the xor function on the file_revilspb_revils_proto_rawDesc object
			if node.Name.Name == revilsPbProtoInitFuncName {
				newStmt := &ast.AssignStmt{
					Lhs: []ast.Expr{
						ast.NewIdent(revilspbVarName),
					},
					Tok: token.ASSIGN,
					Rhs: []ast.Expr{
						&ast.CallExpr{
