...
import (
	...
	"github.com/Kevin-kitnick/revils/implant/revils/transports/mtls"
	"github.com/Kevin-kitnick/revils/implant/revils/transports/wireguard"
...
// Request - Sends a protobuf request to the active revils and returns the response
func (s *Session) Request(msgType uint32, timeout time.Duration, data []byte) ([]byte, error) {
	resp := make(chan *revilspb.Envelope)
	reqID := EnvelopeID()
	...
	select {
	case s.Connection.Send <- &revilspb.Envelope{
		ID:   reqID,
		Type: msgType,
		Data: data,
	}:
	...
	var respEnvelope *revilspb.Envelope
	select {
...
// sessions - Manages the revilss, provides atomic access
...
// Add - Add a revils to the hive (atomically)
...
// Remove - Remove a revils from the hive (atomically)
