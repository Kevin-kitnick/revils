...
// Tunnel  - Essentially just a mapping between a specific client and revils
...
type Tunnel struct {
	...
	FromImplant         chan *revilspb.TunnelData
	FromImplantSequence uint64

	Client rpcpb.RevilsRPC_TunnelDataServer
...
func NewTunnel(id uint64, sessionID string) *Tunnel {
	return &Tunnel{
		...
		FromImplant: make(chan *revilspb.TunnelData),
...
func (t *Tunnel) SendDataFromImplant(tunnelData *revilspb.TunnelData) {
...
func (t *tunnels) Close(tunnelID uint64) error {
	...
	tunnelClose, err := proto.Marshal(&revilspb.TunnelData{
		TunnelID:  tunnel.ID,
	...
	data, err := proto.Marshal(&revilspb.Envelope{
