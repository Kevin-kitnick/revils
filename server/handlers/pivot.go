...
func pivotPeerEnvelopeHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	pivotLog.Debugf("received pivot peer envelope ...")
	peerEnvelope := &revilspb.PivotPeerEnvelope{}
	...
	var resp *revilspb.Envelope
	switch peerEnvelope.Type {
	case revilspb.MsgPivotServerKeyExchange:
		resp = serverKeyExchange(implantConn, peerEnvelope)
	case revilspb.MsgPivotSessionEnvelope:
...
func sessionEnvelopeHandler(implantConn *core.ImplantConnection, peerEnvelope *revilspb.PivotPeerEnvelope) *revilspb.Envelope {
...
func handlePivotEnvelope(pivot *core.Pivot, envelope *revilspb.Envelope) {
	...
	} else if envelope.Type == revilspb.MsgPivotServerPing {
...
func pivotPeerFailureHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	peerFailure := &revilspb.PivotPeerFailure{}
...
// ------------------------
func serverKeyExchange(implantConn *core.ImplantConnection, peerEnvelope *revilspb.PivotPeerEnvelope) *revilspb.Envelope {
	...
	serverKeyEx := &revilspb.PivotServerKeyExchange{}
	...
	keyExRespEnvelope := MustMarshal(&revilspb.Envelope{
		Type: revilspb.MsgPivotServerKeyExchange,
		Data: MustMarshal(&revilspb.PivotServerKeyExchange{
	...
	peerEnvelopeData, _ := proto.Marshal(&revilspb.PivotPeerEnvelope{
	...
	return &revilspb.Envelope{
		Type: revilspb.MsgPivotPeerEnvelope,
...
func peersToString(remoteAddr string, peerEnvelope *revilspb.PivotPeerEnvelope) string {
