...
import (
	...
	"github.com/Kevin-kitnick/revils/server/core/rtunnels"
...
func registerSessionHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	if implantConn == nil {
		return nil
	}
	register := &revilspb.Register{}
...
func auditLogSession(session *core.Session, register *revilspb.Register) {
...
func tunnelDataHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	...
	tunnelData := &revilspb.TunnelData{}
...
func tunnelCloseHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func pingHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func socksDataHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func createReverseTunnelHandler(implantConn *core.ImplantConnection, data []byte) *revilspb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)

	req := &revilspb.TunnelData{}
	...
	if err != nil {
		tunnelClose, _ := proto.Marshal(&revilspb.TunnelData{
			Closed:   true,
			TunnelID: req.TunnelID,
		})
		implantConn.Send <- &revilspb.Envelope{
			Type: revilspb.MsgTunnelClose,
			Data: tunnelClose,
		}
		cancelContext()
		return nil
	...
	if tunnelDataCache.Len(tunnel.ID) > 3 {
		data, err := proto.Marshal(&revilspb.TunnelData{
		...
		if err != nil {
			...
			// {{end}}
			implantConn.RekuestRessend(data)
		}
	}
	return nil
}

func RTunnelDataHandler(tunnelData *revilspb.TunnelData, tunnel *rtunnels.RTunnel, connection *core.ImplantConnection) {
	...
	if tunnelDataCache.Len(tunnel.ID) > 3 {
		...
		if err != nil {
			...
			//sessionHandlerLog.Infof("[tunnel] Requesting resend of tunnelData seq: %d", tunnel.ReadSequence())
			// {{end}}
			connection.RekuestRessend(data)
		}
	}
}
