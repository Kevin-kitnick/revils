...
func (s *Server) Portfwd(ctx context.Context, req *revilspb.PortfwdReq) (*revilspb.Portfwd, error) {
	...
	data, err := session.Request(revilspb.MsgNumber(req), s.getTimeout(req), reqData)
	if err != nil {
		return nil, err
	}
	portfwd := &revilspb.Portfwd{}
