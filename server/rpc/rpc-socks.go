...
var (
	// SessionID->Tunnels[TunnelID]->Tunnel->Cache map[uint64]*revilspb.SocksData{}
	toImplantCacheSocks = socksDataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*revilspb.SocksData{}}

	// SessionID->Tunnels[TunnelID]->Tunnel->Cache
	fromImplantCacheSocks = socksDataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*revilspb.SocksData{}}
)

type socksDataCache struct {
	mutex *sync.RWMutex
	cache map[uint64]map[uint64]*revilspb.SocksData
}

func (c *socksDataCache) Add(tunnelID uint64, sequence uint64, tunnelData *revilspb.SocksData) {
	c.mutex.Lock()
	defer c.mutex.Unlock()

	if _, ok := c.cache[tunnelID]; !ok {
		c.cache[tunnelID] = map[uint64]*revilspb.SocksData{}
	}

	c.cache[tunnelID][sequence] = tunnelData
}

func (c *socksDataCache) Get(tunnelID uint64, sequence uint64) (*revilspb.SocksData, bool) {
	c.mutex.RLock()
...
func (s *Server) SocksProxy(stream rpcpb.RevilsRPC_SocksProxyServer) error {
	for {
			...
			go func() {
				for tunnelData := range socks.FromImplant {
					...
					for recv, ok := fromImplantCacheSocks.Get(fromClient.TunnelID, socks.FromImplantSequence); ok; recv, ok = fromImplantCacheSocks.Get(fromClient.TunnelID, socks.FromImplantSequence) {
						...
						socks.Client.Send(&revilspb.SocksData{
		...
		go func() {
			...
			for recv, ok := toImplantCacheSocks.Get(fromClient.TunnelID, socks.ToImplantSequence); ok; recv, ok = toImplantCacheSocks.Get(fromClient.TunnelID, socks.ToImplantSequence) {
				...
				session.Connection.Send <- &revilspb.Envelope{
					Type: revilspb.MsgSocksData,
					Data: data,
				}
...
func (s *Server) CreateSocks(ctx context.Context, req *revilspb.Socks) (*revilspb.Socks, error) {
...
func (s *Server) CloseSocks(ctx context.Context, req *revilspb.Socks) (*commonpb.Empty, error) {
