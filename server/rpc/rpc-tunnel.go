...
var (
	...
	toImplantCache = dataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*revilspb.TunnelData{}}

	// SessionID->Tunnels[TunnelID]->Tunnel->Cache
	fromImplantCache = dataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*revilspb.TunnelData{}}
)

type dataCache struct {
	mutex *sync.RWMutex
	cache map[uint64]map[uint64]*revilspb.TunnelData
}

func (c *dataCache) Add(tunnelID uint64, sequence uint64, tunnelData *revilspb.TunnelData) {
	c.mutex.Lock()
	defer c.mutex.Unlock()

	if _, ok := c.cache[tunnelID]; !ok {
		c.cache[tunnelID] = map[uint64]*revilspb.TunnelData{}
	}

	c.cache[tunnelID][sequence] = tunnelData
}

func (c *dataCache) Get(tunnelID uint64, sequence uint64) (*revilspb.TunnelData, bool) {
	c.mutex.RLock()
...
func (s *Server) CreateTunnel(ctx context.Context, req *revilspb.Tunnel) (*revilspb.Tunnel, error) {
...
func (s *Server) CloseTunnel(ctx context.Context, req *revilspb.Tunnel) (*commonpb.Empty, error) {
...
func (s *Server) TunnelData(stream rpcpb.RevilsRPC_TunnelDataServer) error {
	for {
		...
		if tunnel.Client == nil {
			...
			tunnel.Client.Send(&revilspb.TunnelData{
				TunnelID:  tunnel.ID,
			...
			go func() {
				...
				data, _ := proto.Marshal(&revilspb.TunnelData{
