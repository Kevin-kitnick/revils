...
import (
	...
	"github.com/Kevin-kitnick/revils/server/msf"
)
...
func (rpc *Server) Msf(ctx context.Context, req *clientpb.MSFReq) (*revilspb.Task, error) {
	var os string
	var arch string
...
func (rpc *Server) MsfRemote(ctx context.Context, req *clientpb.MSFRemoteReq) (*revilspb.Task, error) {
	var os string
	var arch string
	if !req.Request.Async {
		session := core.Sessions.Get(req.Request.SessionID)
