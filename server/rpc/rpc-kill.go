...
func (rpc *Server) Kill(ctx context.Context, kill *revilspb.KillReq) (*commonpb.Empty, error) {
	var (
...
func (rpc *Server) killSession(kill *revilspb.KillReq, session *core.Session) (*commonpb.Empty, error) {
	...
	go session.Request(revilspb.MsgNumber(kill), timeout, data)
	core.Sessions.Remove(session.ID)
	return &commonpb.Empty{}, nil
}

func (rpc *Server) killBeacon(kill *revilspb.KillReq, beacon *models.Beacon) (*commonpb.Empty, error) {
	resp := &commonpb.Empty{}
	...
	task, err := beacon.Task(&revilspb.Envelope{
		Type: revilspb.MsgKillSessionReq,
		Data: reqData,
	})
	if err != nil {
		return nil, err
