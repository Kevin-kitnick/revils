...
func (rpc *Server) KillSession(ctx context.Context, kill *revilspb.KillReq) (*commonpb.Empty, error) {
	...
	session.Request(revilspb.MsgNumber(kill), timeout, data)
	return &commonpb.Empty{}, nil
}

// OpenSession - Instruct beacon to open a new session on next checkin
func (rpc *Server) OpenSession(ctx context.Context, openSession *revilspb.OpenSession) (*revilspb.OpenSession, error) {
	resp := &revilspb.OpenSession{Response: &commonpb.Response{}}
...
func (rpc *Server) CloseSession(ctx context.Context, closeSession *revilspb.CloseSession) (*commonpb.Empty, error) {
	...
	go func() {
		select {
		case session.Connection.Send <- &revilspb.Envelope{Type: revilspb.MsgCloseSession}:
