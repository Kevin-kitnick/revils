...
func GetRootAppDir() string {
	...
	if len(value) == 0 {
		user, _ := user.Current()
		dir = filepath.Join(user.HomeDir, ".revils")
	} else {
		dir = value
	}
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		...
		if err != nil {
			panic("Cannot write to revils root dir")
		}
	}
	return dir
}
...
func rootLogger() *logrus.Logger {
	...
	jsonFilePath := filepath.Join(GetLogDir(), "revils.json")
...
func txtLogger() *logrus.Logger {
	...
	txtFilePath := filepath.Join(GetLogDir(), "revils.log")
...
func (hook *TxtHook) Fire(entry *logrus.Entry) error {
	...
	srcFile := "<no caller>"
	if entry.HasCaller() {
		revilsIndex := strings.Index(entry.Caller.File, "revils")
		srcFile = entry.Caller.File
		if revilsIndex != -1 {
			srcFile = srcFile[revilsIndex:]
		}
	}
